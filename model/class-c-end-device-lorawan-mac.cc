...
ClassCEndDeviceLorawanMac::Receive (Ptr<Packet const> packet)
{
  ...
  //NS_LOG_UNCOND("IF THIS SHOWS UP THE PACKET IS IN ENDDEVICES");

  // Work on a copy of the packet
  Ptr<Packet> packetCopy = packet->Copy ();
  //NS_LOG_UNCOND("Packet Size: "<<packetCopy->GetSize());
      ...
      if (messageForUs)
        {
          ...
          NS_LOG_UNCOND("R "<< Simulator::Now().GetMilliSeconds());
...
//NS_LOG_UNCOND("IF THIS SHOWS UP THE PACKET IS LOST RIP");
