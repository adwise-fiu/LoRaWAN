...
NetworkStatus::GetBestGatewayForDevice (LoraDeviceAddress deviceAddress, int window)
{
  ...
  //NS_LOG_UNCOND("bEFORE mapping status");
...
//std::list<Address> 
//NetworkStatus::GetAvalibleEndDevicesForBroadcast ()
//{
  //NS_LOG_FUNCTION (this);

  //std::list<Address> edAddress = std::list<Address>();
  
  //std::map<Address, Ptr<EndDeviceStatus>>::iterator iter;
  //for (auto &iter : m_endDeviceStatuses)
    //{
	  //edAddress.push_back (iter.first);   
    //}
