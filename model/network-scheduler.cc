...
NetworkScheduler::DoSend(Ptr<Packet> data, LoraDeviceAddress deviceAddress, int window)
{
  ...
  Ptr<Packet> packetCopy = data->Copy ();
	//Here remove the headers
	//LorawanMacHeader receivedMacHdr;
	//packetCopy->RemoveHeader (receivedMacHdr);
	//LoraFrameHeader receivedFrameHdr;
	//receivedFrameHdr.SetAsUplink ();
  ...
  if (deviceAddress.IsBroadcast ()) 
    {
		...
		//NS_LOG_UNCOND("It got recognized as broadcast");
    ...
    // LoraDeviceAddress address = deviceAddress;
		...
		// fHdr.SetFCnt (receivedFrameHdr.GetFCnt ());
		...
		// packetCopy->AddHeader (fHdr);
		// packetCopy->AddHeader (mHdr);
		// packetCopy->AddPacketTag (tag);
    // Create a broadcast frame
    Ptr<Packet> packet = m_status->CreateBroadcastPacket (packetCopy);
  ...
  // Unicast frame
  else 
    {
      ...
      for (auto it = gwAddresses.begin(); it != gwAddresses.end(); it++)
        {
          NS_LOG_DEBUG ("Send a unicast frame through gateway " << *it << " .");
          m_status->SendThroughGateway (packetCopy, *it);
        }
    }
