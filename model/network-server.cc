...
NetworkServer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NetworkServer")
    ...
    .AddAttribute("Delay","Expected computation delay.",
				   DoubleValue (0.0),
                   MakeDoubleAccessor (&NetworkServer::m_delay),
                   MakeDoubleChecker<double>())
    .AddAttribute("Option","0 -> Broadcast; 1-> Unicast",
				   IntegerValue (0),
                   MakeIntegerAccessor (&NetworkServer::m_option),
                   MakeIntegerChecker<int>())
    .AddAttribute("Interval","Required if unicast",
				   DoubleValue (0.0),
                   MakeDoubleAccessor (&NetworkServer::m_interval),
                   MakeDoubleChecker<double>());;
  return tid;
}
...
NetworkServer::AddNode (Ptr<Node> node)
{
  ...
  Ptr<EndDeviceLorawanMac> edLorawanMac1 =
    loraNetDevice->GetMac ()->GetObject<ClassCEndDeviceLorawanMac> ();
    m_addresses.push_back(edLorawanMac1->GetDeviceAddress());
    
   // NS_LOG_UNCOND("Is This the MAC:"<<edLorawanMac->GetObject<ClassCEndDeviceLorawanMac> (););
                        ...
                        uint16_t protocol, const Address& address)
{
  ...
  Ptr<Packet> packetCopy = packet->Copy ();
	...
	if(device->GetObject<PointToPointNetDevice> () != 0){
	...
	// else if (device->GetObject<CsmaNetDevice> () != 0 ){
		
		
	// 	//NS_LOG_UNCOND("NetworkServer Send Here");
	// 	if(m_option == 0){
	// 	Simulator::Schedule(Seconds(m_delay), &NetworkServer::Send, this, ControlCenterPacket, LoraDeviceAddress(0xFF, 0xFFFFFFFF));   //Seconds here mean computation delay
	// 	//Send(packetCopy,LoraDeviceAddress(0xFF, 0xFFFFFFFF));   //Seconds here mean computation delay
	// }
	// 	else if(m_option == 1){
	// 		for (size_t i = 0 ;i<m_addresses.size();i++){
	// 			//NS_LOG_UNCOND("Trying to send to address: "<<m_addresses[i]);

    
	// 			//Simulator::ScheduleNow(&NetworkServer::Send,this,packetCopy,m_addresses[i]);
				
	// 			Simulator::Schedule(Seconds((m_delay+m_interval)*i), &NetworkServer::Send, this, packetCopy, m_addresses[i]);
...
void NetworkServer::SendBroad(Ptr<Packet> data)
{
  LoraDeviceAddress address;
  address.SetNwkID (0x7F);
  address.SetNwkAddr (0x1FFFFFF);
m_scheduler->DoSend(data, address, 2);
}

void NetworkServer::SendUni(Ptr<Packet> data, LoraDeviceAddress deviceAddress)
{
