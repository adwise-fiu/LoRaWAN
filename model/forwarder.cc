                            ...
                            packet, uint16_t protocol, const Address& sender)
{
  ...
  //Ptr<Packet> packetsCopy = packet->Copy ();
  //LorawanMacHeader mHdr;
  //packetsCopy->RemoveHeader (mHdr);
                                    ...
                                    const Address& sender)
{
  ...
  Ptr<Packet> packetCopy = packet->Copy ();
  //NS_LOG_UNCOND("Forwarder is about to send to devices");
  //m_sendEvent = Simulator::Schedule(Seconds(1), &Forwarder::SendPacket,this,packetCopy); 
  
  m_loraNetDevice->Send (packetCopy);   //This is wrong here schedule send function
  //NS_LOG_UNCOND("Forwarder just sent packet");
  return true;
}

void
Forwarder::SendPacket (Ptr<Packet> packet)
{
